...
# ---------- 心情：'ok' 普通 / 'hungry' 肚子餓沒活力 / 'full' 吃飽開心 ----------
MOOD = 'ok'

# 各心情的動畫節奏與姿態（附加在共用 CSS 之後）
MOOD_CSS = {
    'ok': '',
    # 餓：動作變慢、整體微微下沉（沒力氣）
    'hungry': '.bob{animation-duration:3.4s}.rocking{animation-duration:3.6s}'
              '.wag{animation-duration:3s}svg>g{transform:translateY(6px)}',
    # 飽：動作輕快有彈性
    'full': '.bob{animation-duration:1.1s}.rocking{animation-duration:1.4s}'
            '.wag{animation-duration:.9s}'
            '.spark{animation:spark 1.6s ease-in-out infinite}'
            '@keyframes spark{50%{opacity:.2;transform:scale(.7)}}'
            '.drop{animation:drop 2.2s ease-in-out infinite}'
            '@keyframes drop{50%{transform:translateY(4px)}}',
}
MOOD_CSS['hungry'] += '.drop{animation:drop 2.6s ease-in-out infinite}' \
                      '@keyframes drop{0%,100%{transform:translateY(0);opacity:.9}' \
                      '50%{transform:translateY(6px);opacity:.5}}'

def mood_overlay():
    """疊在最上層的心情符號：餓=冒汗珠，飽=閃亮星星。"""
    if MOOD == 'hungry':
        return ('<g class="drop">'
                '<path d="M258 62 Q248 82 258 90 Q270 84 262 62 Z" fill="#7fd8ff" '
                'stroke="#1a1a1a" stroke-width="4" stroke-linejoin="round"/></g>')
    if MOOD == 'full':
        star = ('<path d="M0 -13 L4 -4 L13 0 L4 4 L0 13 L-4 4 L-13 0 L-4 -4 Z" '
                'fill="#ffd60a" stroke="#1a1a1a" stroke-width="3" stroke-linejoin="round"/>')
        return (f'<g class="spark"><g transform="translate(46,70)">{star}</g>'
                f'<g transform="translate(276,92) scale(.8)">{star}</g>'
                f'<g transform="translate(40,222) scale(.7)">{star}</g></g>')
    return ''

def svg(body, anim='bob', static=''):
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 320">'
            f'<style>{CSS}{MOOD_CSS[MOOD]}</style>{static}'
            f'<g class="{anim}">{body}</g>{mood_overlay()}</svg>')

# ---------- 共用零件 ----------
SHADOW = '<ellipse cx="160" cy="298" rx="80" ry="14" fill="#1a1a1a" opacity=".12"/>'

def eyes(y, dx=28, r=10, cx=160):
    """會眨眼的大圓眼；餓=半闔下垂眼，飽=開心彎彎眼。"""
    if MOOD == 'hungry':
        lw = max(4, round(r * 0.62))
        # 上眼皮下垂，只露出下半顆眼睛
        return (f'<g>'
                f'<path d="M{cx-dx-r} {y-2} Q{cx-dx} {y+r} {cx-dx+r} {y-2}" fill="none" '
                f'stroke="#1a1a1a" stroke-width="{lw}" stroke-linecap="round"/>'
                f'<path d="M{cx+dx-r} {y-2} Q{cx+dx} {y+r} {cx+dx+r} {y-2}" fill="none" '
                f'stroke="#1a1a1a" stroke-width="{lw}" stroke-linecap="round"/>'
                # 眼下的疲倦線
                f'<path d="M{cx-dx-r+2} {y+r+4} L{cx-dx+r-2} {y+r+4}" stroke="#1a1a1a" '
                f'stroke-width="3" opacity=".45" stroke-linecap="round"/>'
                f'<path d="M{cx+dx-r+2} {y+r+4} L{cx+dx+r-2} {y+r+4}" stroke="#1a1a1a" '
                f'stroke-width="3" opacity=".45" stroke-linecap="round"/></g>')
    if MOOD == 'full':
        lw = max(5, round(r * 0.72))
        # ^ ^ 開心彎眼
        return (f'<g>'
                f'<path d="M{cx-dx-r} {y+r*0.5} Q{cx-dx} {y-r} {cx-dx+r} {y+r*0.5}" fill="none" '
                f'stroke="#1a1a1a" stroke-width="{lw}" stroke-linecap="round"/>'
                f'<path d="M{cx+dx-r} {y+r*0.5} Q{cx+dx} {y-r} {cx+dx+r} {y+r*0.5}" fill="none" '
                f'stroke="#1a1a1a" stroke-width="{lw}" stroke-linecap="round"/></g>')
    hr = max(3, int(r * 0.35))
...
def smile(y, w=24):
    if MOOD == 'hungry':
        # 波浪嘴（無精打采）
        q = w / 2
        return (f'<path d="M{160-w} {y+4} q{q/2} -7 {q} 0 q{q/2} 7 {q} 0 q{q/2} -7 {q} 0" '
                f'fill="none" {St}/>')
    if MOOD == 'full':
        # 張嘴大笑（含舌頭）
        return (f'<path d="M{160-w} {y-2} Q160 {y+26} {160+w} {y-2} Z" fill="#7a2b2b" {St}/>'
                f'<path d="M{160-w*0.42} {y+13} Q160 {y+26} {160+w*0.42} {y+13} Z" fill="#ff8fa3"/>')
    return f'<path d="M{160-w} {y} Q160 {y+14} {160+w} {y}" fill="none" {S}/>'

def cheeks(y, dx=52, c='#ffb3b3'):
    if MOOD == 'hungry':
        return (f'<circle cx="{160-dx}" cy="{y}" r="9" fill="#c9c9c9" opacity=".35"/>'
                f'<circle cx="{160+dx}" cy="{y}" r="9" fill="#c9c9c9" opacity=".35"/>')
    r = 13 if MOOD == 'full' else 10
    op = '.95' if MOOD == 'full' else '.8'
    return (f'<circle cx="{160-dx}" cy="{y}" r="{r}" fill="{c}" opacity="{op}"/>'
            f'<circle cx="{160+dx}" cy="{y}" r="{r}" fill="{c}" opacity="{op}"/>')
...
# 心情 → 檔名後綴（普通維持原檔名，不影響既有引用）
MOOD_SUFFIX = (('ok', ''), ('hungry', '_h'), ('full', '_f'))

def main():
    os.makedirs(OUT, exist_ok=True)
    n = 0
    for mood, suffix in MOOD_SUFFIX:
        for key, content in build_all(mood).items():
            with open(os.path.join(OUT, f'pet_{key}{suffix}.svg'), 'w', encoding='utf-8') as f:
                f.write(content)
            n += 1
    print(f'done: {n} pet SVGs ({len(MOOD_SUFFIX)} moods) -> {OUT}')
